	...
	if (!m_isConnected) {
		try {
			...
			if (m_Root) {	// Try to get element handles
				...
				// Attempt to obtain speed/acceleration interface via QueryInterface
				try {
					m_stageXSpeed = m_stageX;
					m_stageYSpeed = m_stageY;
				} catch (_com_error& e) {
					qDebug() << "IMTBContinualSpeed not available for stage axes:" << e.ErrorMessage();
				}
			}
...
// NOTE: Speed unit is kUnitMicrometerPerSec and acceleration unit is kUnitMicrometerPerSecSq — verify against MTB Control if values look unexpected.
...
double ZeissMTB_Erlangen2::getStageSpeed() {
	if (!m_stageXSpeed) {
		return -1.0;
	}
	try {
...
double ZeissMTB_Erlangen2::getMinStageSpeed() {
	if (!m_stageXSpeed) {
		return -1.0;
	}
	try {
...
double ZeissMTB_Erlangen2::getMaxStageSpeed() {
	if (!m_stageXSpeed) {
		return -1.0;
	}
	try {
...
void ZeissMTB_Erlangen2::setStageAcceleration(double acceleration) {
	if (!m_stageXSpeed || !m_stageYSpeed) {
		return;
	}
	try {
...
double ZeissMTB_Erlangen2::getStageAcceleration() {
	if (!m_stageXSpeed) {
		return -1.0;
	}
	try {
...
double ZeissMTB_Erlangen2::getMinStageAcceleration() {
	if (!m_stageXSpeed) {
		return -1.0;
	}
	try {
...
double ZeissMTB_Erlangen2::getMaxStageAcceleration() {
	if (!m_stageXSpeed) {
		return -1.0;
	}
	try {
